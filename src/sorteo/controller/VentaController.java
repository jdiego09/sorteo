...
public class VentaController extends Controller implements Initializable {
    ...
    void impBoletaVentas(ActionEvent event) {
        if (this.factura.getSorteo() == null || this.factura.getSorteo().getCodigo() == null || this.factura.getSorteo().getCodigo() <= 0) {
            AppWindowController.getInstance().mensaje(Alert.AlertType.ERROR, "Sorteo no seleccionado", "No ha seleccionado un sorteo válido.");
            return;
        }
        SorteoDao.getInstance().genListaNumeros(this.factura.getSorteo().getTipoSorteo().getCodigo());
        HashMap<String, Object> parametros = new HashMap<>();
        parametros.put("sorteo", this.factura.getSorteo().getCodigo());
        Aplicacion.getInstance().generarReporte("rpt_resVentaSorteo", parametros);
    }
