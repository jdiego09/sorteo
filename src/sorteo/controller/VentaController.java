...
public class VentaController extends Controller implements Initializable {
   ...
   private void init() {
      ...
      tbvDetFactura.setRowFactory(t -> {
         TableRow<DetalleFactura> fila = new TableRow<>();
         fila.setOnMouseClicked((MouseEvent event) -> {
            if (fila.getItem() != null) {
               detalleSeleccionado = fila.getIndex();
            }
         });
         return fila;
      });
   ...
   final EventHandler<ActionEvent> numeroHandler = (final ActionEvent event) -> {
      ...
      Resultado<DetalleFactura> numero = new Resultado<>();
      numero.set(detalleFactura.get(detalleFactura.size()));
      Aplicacion.getInstance().setMontoIngresado(numero);
      event.consume();
   };
   ...
   private void returnMonto() {
      if (txtNumero.getText().length() > 0) {
         ...
         AppWindowController.getInstance().mensaje(Alert.AlertType.WARNING, "Monto incorrecto", "Debe indicar un monto a apostar");
         txtNumero.requestFocus();
      }
   }
